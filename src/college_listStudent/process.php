...
if (isset($_POST['update'])){
    ...
    $final = $_POST['final'];
      if($final == 1){
        ...
        $mail->Body = 'The following Student '.$userid.' has changed Major to '.$major.' and Decision Plan to '.$decision.' for school '.$school;
        if (!$mail->send()) {
    ...
    $mysqli->query("UPDATE list SET major='$major', decision='$decision', final='$final' WHERE id=$id") or
            die($mysqli->error);
    $_SESSION['message'] = "Record has been updated!";
    $_SESSION['msg_type'] = "warning";
